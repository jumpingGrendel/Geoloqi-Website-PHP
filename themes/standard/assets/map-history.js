...
var LQHistory = {
	...
	get_history: function(){
		...
		if($("#history_time_from").val()){
			params.after = $("#history_from").val();
		}
		if($("#history_time_to").val()){
			params.before = $("#history_to").val();
		}
		...
		$.getJSON("/map/history.ajax", params, function(data){
			...
			for(var i=0; i<data.length; i++){
				path.push(new google.maps.LatLng(data[i].location.position.latitude, data[i].location.position.longitude));
			}
